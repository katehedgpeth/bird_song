defmodule BirdSongWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, BirdSongWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      # this function should not set a response
      refute conn.status
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      BirdSongWeb.Endpoint.subscribe(live_socket_id)
    ...
    test "works even if user is already logged out", %{conn: conn} do
      ...
      # method should not set a response
      refute conn.status
      assert conn.state == :unset
